...
(function($){
	...
	function enterTreeStuff() {
		...
		.fromTo($nest, 1, {y:10, scale: 0.2, autoAlpha: 0, transformOrigin: 'center center'}, 
			{y:'-=15', scale: 1, autoAlpha: 1, transformOrigin: 'center center', ease: Elastic.easeout}, '+=0.1')
		.to($nest, 0.3, {y: '+=4', ease: Bounce.easeOut}, '-=0.2') // Start 0.2 earlier
		.add('nest-pop-in')
		...
		.to($birdHat, 0.4, {y: 0, x: 0, rotation: 0, onComplete: startBlinking}, '-=0.2')
		;
	...
	function enterGreeting() {
		var greetingTl = new TimelineMax();
			...
			function startLoops() {
				// start background color startLoops
